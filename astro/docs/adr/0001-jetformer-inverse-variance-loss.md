...
### Spectra, affine + Huber (cheap)

`spectrum.ivar` already flows through `data/mmu.py`. The PLAN lists this
as a config option ("Option (not pilot-default): ivar-weighted Huber").
If spectra are ever moved off the jetformer head to the Huber head, lift
`spectrum_jeff.loss_fn`'s
...
### Images, affine + Huber (viable, but the data switch is the cost)

The affine + Huber path (`tokeniser: affine`, the default) is a
point-prediction loss, so ADR 0002's core argument — don't spend
capacity reproducing noise — applies to images exactly as it does to
`galactiktok`'s MSE. ivar-weighting composes with Huber cleanly:
`huber(√w·(ŷ−y), delta)` is a robustified Gaussian likelihood, sound in
the small-residual (weighted-L2) regime and down-weighting outliers in
the large-residual regime. The 3 image bands are packed into one
192-dim patch token, so the ADR's per-band reduced-χ² averaging becomes
per-component weighting *inside* the token — the band-subset-invariance
machinery is irrelevant here (no band dropout), but the noise weighting
carries over. **Verdict: scientifically sound and implementable.**

The catch is that the current pilot survey (`mmu_ssl_legacysurvey_north`)
ships **no per-pixel image ivar** — only per-band `psfdepth` catalog
scalars — so enabling image ivar means switching the pilot to
`hugging-science/mmu_legacysurvey_dr10_south_21` (schema verified from
the dataset page, 2026-07-15: `image.flux` / `image.ivar` / `image.mask`,
160×160 px, 4 bands `des-g/r/i/z`, 124M rows, 61 TiB). That switch is
the real work, not the loss edit:

- **Reverses PLAN user decision #4** (pinned to north). Acceptable if
  re-evaluating the pilot data anyway; not worth doing *only* to enable
  ivar-weighting.
- **Shape change 152→160 px.** Patch 8 goes 19→20 → **400 tokens vs
  361**, touching `tokenization.py`, packing, configs, and every
  seq-length assumption. Alternatively crop/resample 160→152 to keep
  the pipeline constant (the noise-weighting doesn't care about the
  crop). Either is bounded but real.
- **3→4 bands** changes `input_size` 192→256 and every modality
  encoder/decoder width, *or* take g/r/z off the 4-band set and stay
  shape-compatible (minimises ripple, forfeits `des-i`).
- **9× the rows** (14M→124M) — longer prep, larger corpus (good for the
  larger sizes Phase 6 targets).
- **South footprint vs DESI SV3** — the image×spectra crossmatch yield
  and the image-only/image+spectra ratio change; re-eval before
  committing.
- **Not weight-compatible** with any north-trained checkpoint. Cheap
  now (pre-release; Phase 5 shakeouts are image-only-on-north and not
  releasable), expensive once a real pilot trains.

One genuinely new piece of plumbing vs the ADR: this repo's affine path
applies **per-patch standardization** after the asinh stretch
(`transforms.per_patch_standardize`), which the `galactiktok` tokenizer
does not. Standardization rescales each patch by its own `σ_patch`, so
raw-flux ivar must be propagated into the loss space:
`ivar_loss = ivar_pixel · σ_patch²` (the squared Jacobian of
`(a−μ)/σ`). This is a **fixed per-patch scalar**, computed anyway during
the forward transform — cheap and exact, unlike the jetformer's learned
flow Jacobian (rationale #2 above). The asinh Jacobian itself (ADR
Option C, `ivar · (scale² + (f−offset)²)`) is the same second-order
correction the ADR defers; the sky-dominated regime where it ≈ 1 is the
common case here too. So the loss-side change is small: weight the
Huber residual by `√(clamp(ivar·σ_patch²))` per component, zero where
`image.mask` flags a pixel (folded into `ivar=0` at load time, as the
ADR specifies). The data switch dominates the effort.

For the jetformer itself: skip it. The density head is the ivar.

## Consequences

- **Jetformer head: no change** to `modeling_astropt3.py`,
  `generation.py`, the data pipeline, or the nanotron fork's loss
  block. The objective stays a valid exact log-likelihood, so
  sampling/generation and the `starts-1` GMM alignment remain
  consistent with training.
- **Affine + Huber, spectra:** ivar-weighted Huber is a one-line
  option once spectra use the Huber head; ivar is already in the
  record.
- **Affine + Huber, images:** viable, contingent on the DR10-south
  pilot switch (reverses PLAN decision #4). The loss edit is small
  (propagate ivar through per-patch standardization, which is a free
  `· σ_patch²`); the data switch (160 px, 4 bands, south footprint,
  9× volume, crossmatch re-eval) is the real cost. Recommend doing it
  only if the pilot survey is being re-pinned anyway, not as a
  loss-only change.
